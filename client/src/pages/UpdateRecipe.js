...
import { getDiets, getRecipesById } from '../redux/Actions'
...
const UpdateRecipe = () => {
    let {id} = useParams()
    let recipe = useSelector(state => state.getRecipeByID)
    console.log(recipe)
    ...
    let [input, setInput] = useState({
        ...
        summary: !!recipe.summary?recipe.summary:'', 
        image: !!recipe.image?recipe.image:'', 
        healthScore: !!recipe.healthScore?recipe.healthScore:0, 
    ...
    const handleSubmit = (e) => {
        try {
            ...
            if(input.name && input.summary && input.image && !!input.diets.length && !!input.steps.length){
                ...
                axios.put('http://localhost:3001/recipes/'+ id, input)
                .then(res => alert('Recipe Updated'))
                history.push('/Home')
            }else alert('Some fields are missing')
        } catch (error) {
            alert("It coudn't be Updated")
        }
    }
    ...
    return (
        <div className={s.createRecipeContainer}>
            <form >
                ...
                <input type='submit' value='Update Recipe' onClick={handleSubmit}/>
